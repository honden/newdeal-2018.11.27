...
//mybatis의 DAO 인터페이스의 구현체를 자동 생성 도우미 추가한다
//DAO인터페이스가 들어있는 패키지를 지정한다
//자동생성된 DAO구현체가 SQL을 찾을 때 인터페이스에 전체이름으로 찾는다\
//따라서 sql mapper 파일에 namespace이름은 인터페이스의 전체 이름과 같아야 한다
//인터페이스의 메서드 이름, 메서드 파라미터, 리턴타입이 sql메퍼의 메서드 이름, 파라미터 타입, 리턴타입과 같아야 한다,
